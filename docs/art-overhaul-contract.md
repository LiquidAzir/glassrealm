...
## Second-pass addendum (2026-09-08)

The user rejected the remaining visual quality after the first release and authorized another full pass followed by production deployment. The new baseline is cac4ffc. Independent review must address observed player occlusion and UI overlap, rather than scoring only relative improvement. Check twelve headings at spawn, plaza, close pine and store; four headings inside all ten interior types; screen/glasses modes; crowded labels; hero equipment and movement. Preserve resource indices, exact coordinates/heights, collision and save semantics. Keep the 600px buffer, 30Hz draw cap, shared low-poly assets and absence of dynamic shadows/postprocessing. Record actual costs and remaining physical-device limitations. Release only after the independent checks and actual screenshots support the result.

Second-pass ownership supersedes the initial list: the interface agent owns styles.css, ui.js and main.js updateMarkers(); the asset agent owns Blender sources/kit and the final special-interior polish in interiors.js; root owns other integration and release work. The critic owns independent evidence in `.visual-review/realm-round2/verification/` and does not edit game files or deploy. The practical visual target is 8/10 across the sampled scenes, with no critical/high visibility or interface issues. Do not convert incomplete coverage into a claim that every region was inspected.
