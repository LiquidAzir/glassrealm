...
The script writes the compact runtime JSON, an editable `source/realm-kit.blend`, and real Blender renders plus metrics under the workspace's `.visual-review/realm/assets/`. All generation uses a fixed random seed. Each asset is a named joined mesh in the Blender source; environment and furniture assets have separate contact sheets, and the character meshes have an assembly study. Export happens before the display layout is applied. The saved scene opens with the character study visible; original source meshes are retained, hidden for viewport/render clarity, and can be unhidden in Blender.
...
Character origins differ from grounded environment props: torso, cuirass, head and shoulder are centered for attachment to existing runtime pivots; the boot starts at its hip pivot, Y=0, and extends to Y=-0.70. All parts except the head use strictly grayscale colors so runtime cloth and armor materials can tint them. The head retains baked skin, hair and eye colors. Its bottom is Y=-0.265; attach it close enough to the existing collar/neck to avoid a gap. Complete bounds are in `realm-kit.json` metadata. Arms, hands, belt and sword in the Blender character study are assembly context, not extra exported models or animation code.
...
Runtime JSON size: 2,399,547 bytes uncompressed (about 2.29 MiB), 18,408 triangles across all 20 models. Geometry checks cover finite arrays, index bounds, linear color ranges, unit normals, triangle winding, nondegenerate triangles, ground origins, building and furniture bounds, entry clearance, character grayscale tinting, and triangle budgets. SHA-256 comparisons verify that adding the three furnishings preserved all 17 existing model geometries exactly. Blender contact sheets verify authored forms; the independent game review must verify their appearance and performance in the actual renderer. Blender's contact-sheet lighting includes soft shadows for asset inspection; runtime lighting remains the responsibility of the game renderer. The roof palette was narrowed after independent review to remove distracting checkerboard contrast while preserving shingle courses.
