// GlassRealm cloud-save Worker — a tiny per-account key/value store on Cloudflare KV.
// GET  /v1/save?u=<id>  -> { t, data }   (data is the save snapshot, or null)
// PUT  /v1/save?u=<id>  body=<snapshot>  -> { ok: true }
...
export default {
  async fetch(req, env) {
    ...
    if (!u.pathname.endsWith('/v1/save')) return new Response('GlassRealm save worker', { headers: cors });
