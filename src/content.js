...
export const ENEMIES = {
  ...
  the_dreamward:       { name: 'The Dreamward',        hp: 230, dmg: 28, speed: 3.4, xp: 720,  color: 0x4fd6dc, aggro: 18, shape: 'humanoid', scale: 1.9, boss: true, loot: { gold: 300, dreamlight_lantern: 1, tideglass: 4, pearl: 2, bones: 3 }, rare: { item: 'tideglass_ring', chance: 0.3 } },
  ...
  heart_of_hoarfrost:  { name: 'The Heart of Hoarfrost', hp: 300, dmg: 32, speed: 3.0, xp: 1000, color: 0x9bf2ff, aggro: 20, shape: 'beast', scale: 2.2, boss: true, loot: { gold: 420, skyfire_quartz: 1, aurora_quartz: 4, sapphire: 2, bones: 4 } },
  the_reckoner:        { name: 'The Reckoner',         hp: 300, dmg: 34, speed: 3.2, xp: 1050, color: 0xc8a6ff, aggro: 20, shape: 'humanoid', scale: 2.2, boss: true, loot: { gold: 440, voidwarden_cloak: 1, amethyst_wisp: 4, emerald: 2, bones: 4 } },
};
...
function questGiver(name, qid, cfg) {
  return { root: (G) => {
    ...
    if (st === 'available') return node(name, cfg.intro, [{ label: cfg.accept, action: (g) => g.quests.accept(qid), to: null }, end('Not yet.')]);
    if (st === 'active') {
      if (G.quests.isReady(qid)) return node(name, cfg.done, [{ label: 'Claim reward.', action: (g) => g.quests.complete(qid), to: null }]);
      const rem = G.quests.objectives(qid).filter((o) => !o.done).map((o) => '• ' + o.text).join('   ');
      return node(name, cfg.active + '  Remaining:  ' + rem, [end('On it.')]);
    }
    return node(name, cfg.greet, [end('Farewell.')]);   // complete, or locked behind an earlier stage
  } };
}
